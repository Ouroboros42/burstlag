...
class KnownValuesTest(unittest.TestCase):
    def test(self):
        ...
        a2 = np.array([0, 1, 2, 1, 0], dtype=np.float64)
        a3 = np.array([1, 2, 1, 0, 0], dtype=np.float64)

        rel = DetectorRelation(0.1, 0.1, int(sum(a1)), int(sum(a2)), len(a1))
        ...
        self.assertAlmostEqual(-2.6169070459462507,
            log_likelihood(cache, rel, a1, a3, 0.1)
        )
